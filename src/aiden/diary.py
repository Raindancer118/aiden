...
AIDEN deliberately talks to Diary through its public MCP interface. It
...
class DiaryBridgeSettings:
    ...
    def from_environment(cls) -> "DiaryBridgeSettings":
        """Build settings from AIDEN's Diary environment variables."""
        command_line = os.environ.get("AIDEN_DIARY_COMMAND", "diary-mcp-local")
        parts = shlex.split(command_line)
        if not parts:
            raise DiaryBridgeError("AIDEN_DIARY_COMMAND must contain an executable")

        timeout_raw = os.environ.get("AIDEN_DIARY_TIMEOUT_SECONDS", "30")
        try:
            timeout_seconds = float(timeout_raw)
        except ValueError as exc:
            raise DiaryBridgeError("AIDEN_DIARY_TIMEOUT_SECONDS must be a number") from exc
        if timeout_seconds <= 0:
            raise DiaryBridgeError("AIDEN_DIARY_TIMEOUT_SECONDS must be positive")
...
class DiaryMcpBridge:
    ...
    def _start_locked(self) -> BlockingPortal:
        ...
        portal_context = start_blocking_portal(name="aiden-diary-mcp")
        portal = portal_context.__enter__()
        self._ready = ThreadEvent()
        self._startup_error = None
        serve_future = portal.start_task_soon(self._serve, name="aiden-diary-session")
    ...
    def call(self, tool_name: str, arguments: Optional[dict[str, Any]] = None) -> str:
        """Call a Diary MCP tool from AIDEN's synchronous tool runtime."""
        with self._lock:
...
def get_diary_bridge() -> DiaryMcpBridge:
    """Return AIDEN's process-wide Diary MCP bridge."""
