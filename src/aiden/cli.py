"""Command-line entrypoint for AIDEN.

This wrapper registers AIDEN's tools before Serena's registry singleton is
created. It also removes Serena's local Markdown memory workflow: AIDEN
...
def register_aiden_tools() -> None:
    """Make AIDEN's tools discoverable by Serena's ToolRegistry.

    Serena's ``ToolRegistry`` only registers ``Tool`` subclasses whose module
    starts with one of the package prefixes in
    ``serena.tools.tools_base.tool_packages``. We append ``"aiden.tools"``
    ...
    if "aiden.tools" not in tools_base.tool_packages:
        tools_base.tool_packages.append("aiden.tools")

    # Importing the package forces all AIDEN Tool subclasses to be defined,
    # which is required for ToolRegistry's subclass discovery to find them.
    import aiden.tools  # noqa: F401


def _explorer_enabled() -> bool:
    """Whether to bring the graph explorer up with the MCP server."""
    return os.environ.get("AIDEN_EXPLORER", "1").strip().lower() not in ("0", "false", "no", "off")
...
def start_explorer_for(project_root: str) -> None:
    ...
    if not _explorer_enabled():
        return
    try:
        from aiden.web.explorer import ensure_explorer

        ensure_explorer(project_root)
    except Exception as e:  # pragma: no cover - defensive
        log.warning("Could not start the AIDEN explorer: %s", e)
...
def _hook_project_activation() -> None:
    ...
    if getattr(ActivateProjectTool, "_aiden_explorer_hooked", False):
        return
...
def _hook_explorer_notices() -> None:
    ...
    def wrap(tool_cls: type) -> None:
        ...
        tool_cls._aiden_notice_hooked = True  # type: ignore[attr-defined]

    def walk(cls: type) -> None:
        for subclass in cls.__subclasses__():
            if subclass.__module__.startswith("aiden.tools"):
                wrap(subclass)
            walk(subclass)
