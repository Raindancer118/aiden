"""A single shared explorer server that every AIDEN instance attaches to.

Serena starts one dashboard per agent, each on its own port. That is the wrong
shape here: a developer runs AIDEN in several repositories at once and
...
#: Every project AIDEN has ever been used in, so the explorer can list
#: them when no instance is currently attached to them.
_HISTORY_FILE = Path.home() / ".aiden" / "projects.json"
...
_TOKEN_FILE = Path.home() / ".aiden" / "token"
_TOKEN_HEADER = "X-AIDEN-Token"
...
def is_own_aiden_process(pid: int) -> bool:
    """Whether ``pid`` is a live AIDEN process belonging to this user.
    ...
    try:
        ...
        return False
    return "aiden" in haystack or pid == os.getpid()
...
class ProjectHistory:
    """Projects AIDEN has run in before, remembered across restarts.
...
def build_app(registry: ProjectRegistry):  # type: ignore[no-untyped-def]
    ...
    #: same-origin page (or a local process reading ~/.aiden/token) has.
    ...
    def index():  # type: ignore[no-untyped-def]
        ...
        return Response(page.replace("__AIDEN_TOKEN__", explorer_token()), mimetype="text/html")
    ...
    def known_projects():  # type: ignore[no-untyped-def]
        """Projects AIDEN has run in, whether attached right now or not."""
        attached = {p.root: p for p in registry.all()}
        rows = []
        for entry in registry.history.all():
            root = entry["root"]
            index_db = Path(root) / ".serena" / "aiden" / "index.db"
    ...
    def shutdown():  # type: ignore[no-untyped-def]
        ...
        for project in registry.all():
            ...
            if not pid or pid == os.getpid():
                continue
            if not is_own_aiden_process(pid):
                ...
                refused.append({"project": project.name, "pid": pid, "reason": "not a verifiable AIDEN process"})
                continue
            try:
    ...
    def run_action(project_id: str, action: str):  # type: ignore[no-untyped-def]
        ...
        threading.Thread(target=worker, name=f"aiden-action:{action}", daemon=True).start()
...
def _action_sync(root: str, _payload: dict) -> str:
    from aiden.index.incremental import sync_incremental
    from aiden.index.indexer import Indexer
...
def _action_watch_start(root: str, _payload: dict) -> str:
    from aiden.index.watcher import start_watcher, watcher_status
...
def _action_watch_stop(root: str, _payload: dict) -> str:
    from aiden.index.watcher import stop_watcher, watcher_status
            ...
            entries.append(
                {
                    ...
                    "indexed": (entry / ".serena" / "aiden" / "index.db").exists(),
                }
            )
...
def _clones_payload(root: str, min_lines: int = 8, similarity: float = 0.92) -> dict:
    ...
    from aiden.index.search import SearchEngine
...
def _overview_payload(root: str, limit: int = 40) -> dict:
    ...
    from aiden.index.indexer import Indexer, default_db_path
    from aiden.index.search import SearchFilter, _filter_sql
    from aiden.index.store import IndexStore
...
def stop_this_process(delay_s: float = 0.5) -> None:
    ...
    threading.Thread(target=stop, name="aiden-explorer-stop", daemon=True).start()
...
def ensure_explorer(
    ...
    *,
    ...
    with _server_lock:
        ...
        if not _port_is_free(host, port):
            ...
            for _ in range(_ATTACH_RETRIES):
                ...
                log.warning("Port %d is in use but is not an AIDEN explorer; not starting one.", port)
                return url, False
    ...
    if not _wait_until_serving(host, port):
        ...
        return url, True
    log.info("AIDEN explorer running at %s", url)
    if open_browser:
        _open_browser(url)
    return url, True
