"""Diary-backed memory tools for AIDEN.

The local Markdown memory tools inherited from Serena are disabled by the
AIDEN entrypoint. These tools proxy Diary's public MCP API instead.
"""
...
from aiden.diary import get_diary_bridge, project_slug
...
class MemoryProjectContextTool(Tool):
    """Return Diary context for the active AIDEN project."""
...
class MemoryGetTool(Tool, ToolMarkerDoesNotRequireActiveProject):
    ...
    def apply(self, path: str) -> str:
        ...
        :param path: absolute Diary path such as ``/projects/aiden/status``.
