...
class ReindexTool(Tool):
    """
    Build or refresh the AIDEN hybrid index for the active project.
    ...
    SQLite database under ``.serena/aiden/index.db``. Indexing is
    ...
    def apply(self, force: bool = False, embeddings: bool = True) -> str:
        ...
        :param force: if true, reindex every file even if its content hash is
            unchanged (use after upgrading AIDEN or changing the embedder).
...
class IndexStatusTool(Tool):
    """
    Report what the AIDEN index can currently answer, and what it cannot.
