r"""``aiden index`` - build and inspect the hybrid index from the shell.

Indexing is the one long, memory-hungry operation AIDEN performs, and it
...
from aiden.index.embed import DEFAULT_BATCH_SIZE, get_embedder
...
def index_group() -> None:
    """Build, refresh and inspect the AIDEN hybrid index."""
...
@click.option(
    ...
    help="Upper bound on documents per forward pass. For memory, set AIDEN_EMBED_BATCH_COST instead: that budget, not this cap, is what sets the peak.",
)
@_json_option
@_verbose_option
...
def explorer(project: Path, no_browser: bool, port: int | None) -> None:
    ...
    from aiden.web.explorer import EXPLORER_PORT, ensure_explorer

    url, started = ensure_explorer(project, open_browser=not no_browser, port=port or EXPLORER_PORT)
    click.echo(f"{'Started' if started else 'Attached to'} the AIDEN explorer at {url}")
    if not started:
...
def search(project: Path, query: str, limit: int, lang: str, kind: str, path_glob: str, exclude_tests: bool, as_json: bool) -> None:
    ...
    from aiden.index.search import SearchFilter
    ...
    if as_json:
        ...
        return
    if not hits:
        click.echo("No matches. Is the index built? (aiden index build)")
        return
    for hit in hits:
