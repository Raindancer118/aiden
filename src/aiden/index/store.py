"""SQLite persistence for the AIDEN index.
...
from aiden.index.parser import RefHit, SymbolDef
...
class IndexStore:
    ...
    def _migrate(self) -> None:
        ...
        log.info("Migrated the AIDEN index from schema %d to %d.", current, SCHEMA_VERSION)
    ...
    def upsert_file(
        ...
        """
        cur = self.conn.cursor()
        cur.execute("SAVEPOINT aiden_upsert_file")
        try:
            self._delete_file_rows(path)
            cur.execute(
            ...
            cur.execute("ROLLBACK TO SAVEPOINT aiden_upsert_file")
            cur.execute("RELEASE SAVEPOINT aiden_upsert_file")
            raise
        cur.execute("RELEASE SAVEPOINT aiden_upsert_file")
        return inserted
