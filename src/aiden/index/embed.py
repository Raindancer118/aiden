...
_ENV_MODEL = "AIDEN_EMBED_MODEL"
_ENV_BATCH = "AIDEN_EMBED_BATCH"
_ENV_BATCH_COST = "AIDEN_EMBED_BATCH_COST"
_ENV_THREADS = "AIDEN_EMBED_THREADS"
...
class Embedder(ABC):
    ...
    def embed_batched(self, texts: list[str]) -> Iterator[list[tuple[int, list[float]]]]:
        ...
        *A fixed item count.* That reuse only holds if later tensors are
        ...
        ``AIDEN_EMBED_BATCH_COST`` to trade memory back for speed.
        """
        if not texts:
            return
...
def get_embedder(name: str = "auto", **kwargs: object) -> Embedder:
    ...
    The model name can be overridden with ``AIDEN_EMBED_MODEL``.
    """
