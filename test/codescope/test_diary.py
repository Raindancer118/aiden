"""Tests for Codescope's mandatory Diary MCP backend."""
...
from codescope.diary import DiaryBridgeError, DiaryBridgeSettings, DiaryMcpBridge, project_slug


def test_settings_parse_command_and_timeout(monkeypatch: pytest.MonkeyPatch) -> None:
    monkeypatch.setenv("CODESCOPE_DIARY_COMMAND", 'python -m "diary server"')
    monkeypatch.setenv("CODESCOPE_DIARY_TIMEOUT_SECONDS", "12.5")
...
def test_settings_reject_invalid_timeout(monkeypatch: pytest.MonkeyPatch, value: str) -> None:
    monkeypatch.setenv("CODESCOPE_DIARY_TIMEOUT_SECONDS", value)
...
def test_project_slug_normalizes_directory_name() -> None:
    assert project_slug("/tmp/SE Projects/Volantic Codescope") == "volantic-codescope"
...
""".lstrip(),
    ...
    try:
        assert bridge.call("memory_get", {"path": "/projects/codescope/status"}) == "diary:/projects/codescope/status"
