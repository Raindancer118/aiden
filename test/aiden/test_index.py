"""Tests for the AIDEN index engine (parser + store + indexer)."""
...
from aiden.index.parser import SymbolDef, TreeSitterParser
...
# -- surviving the rename from Codescope to AIDEN ----------------------------


def test_an_index_from_before_the_rename_is_adopted(tmp_path: Path) -> None:
    """A pre-rename index is minutes of work; move it rather than rebuild it."""
    from aiden.index.indexer import LEGACY_INDEX_RELPATH, adopt_legacy_index, default_db_path

    legacy = tmp_path / LEGACY_INDEX_RELPATH
    legacy.parent.mkdir(parents=True)
    legacy.write_bytes(b"an index")
    legacy.with_name(legacy.name + "-wal").write_bytes(b"wal")

    adopted = adopt_legacy_index(tmp_path)

    assert adopted == default_db_path(tmp_path)
    assert adopted.read_bytes() == b"an index"
    assert adopted.with_name(adopted.name + "-wal").exists()
    assert not legacy.exists()


def test_an_existing_index_is_never_overwritten_by_the_old_one(tmp_path: Path) -> None:
    from aiden.index.indexer import LEGACY_INDEX_RELPATH, adopt_legacy_index, default_db_path

    legacy = tmp_path / LEGACY_INDEX_RELPATH
    legacy.parent.mkdir(parents=True)
    legacy.write_bytes(b"old")
    current = default_db_path(tmp_path)
...
def test_adopting_is_a_no_op_without_a_legacy_index(tmp_path: Path) -> None:
