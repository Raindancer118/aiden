...
from aiden.index.incremental import git_changes, sync_incremental
...
def test_watcher_picks_up_changes(tmp_path: Path) -> None:
    from aiden.index.watcher import start_watcher, stop_watcher, watcher_status

    (tmp_path / "seed.py").write_text("def seed():\n    pass\n")
    db = tmp_path / ".serena" / "aiden" / "index.db"
    try:
