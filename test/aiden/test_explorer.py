...
from aiden.web import explorer
from aiden.web.notices import format_events
...
def client(project: Path):  # type: ignore[no-untyped-def]
    ...
    client.environ_base["HTTP_X_AIDEN_TOKEN"] = explorer.explorer_token()
...
def test_shutdown_signals_instances_then_itself(client, monkeypatch: pytest.MonkeyPatch) -> None:  # type: ignore[no-untyped-def]
    ...
    monkeypatch.setattr(explorer, "is_own_aiden_process", lambda pid: True)
...
def test_state_changing_routes_require_the_token(client) -> None:  # type: ignore[no-untyped-def]
    ...
    for path in ("/api/projects", "/api/shutdown", f"/api/projects/{project_id}/actions/sync", "/api/known/forget"):
        response = http.post(path, json={"root": "/tmp"}, headers={"X-AIDEN-Token": "wrong"})
        assert response.status_code == 403, f"{path} accepted a bad token"
        assert http.post(path, json={"root": "/tmp"}, headers={"X-AIDEN-Token": ""}).status_code == 403
...
def test_a_foreign_origin_is_rejected(client) -> None:  # type: ignore[no-untyped-def]
    ...
    response = http.post(
        "/api/shutdown",
        headers={"X-AIDEN-Token": explorer.explorer_token(), "Origin": "https://evil.example"},
    )
...
def test_registration_is_confined_to_the_home_directory(client) -> None:  # type: ignore[no-untyped-def]
    http, _registry, _project_id = client
    response = http.post("/api/projects", json={"root": "/etc"}, headers={"X-AIDEN-Token": explorer.explorer_token()})
    assert response.status_code == 400
...
def test_shutdown_refuses_a_pid_it_cannot_vouch_for(project: Path, monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    monkeypatch.setattr(explorer, "is_own_aiden_process", lambda pid: False)

    payload = app.test_client().post("/api/shutdown", headers={"X-AIDEN-Token": explorer.explorer_token()}).get_json()
...
def test_process_verification_accepts_this_process_and_rejects_pid_one() -> None:
    assert explorer.is_own_aiden_process(__import__("os").getpid()) is True
    # pid 1 is init: a different user and not an AIDEN process.
    assert explorer.is_own_aiden_process(1) is False
...
def test_the_page_carries_the_token_so_the_browser_can_use_it(client) -> None:  # type: ignore[no-untyped-def]
    ...
    assert "__AIDEN_TOKEN__" not in body
